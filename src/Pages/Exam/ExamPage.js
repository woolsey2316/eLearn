...
function ExamPage() {
  ...
  const [answerOptions, setAnswerOptions] = useState(quizQuestions[0].possibleAnswers)
  const [answer, setAnswer] = useState('')
  const [selectedOption, setSelectedOption] = useState('')
  const [answerList, setAnswerList] = useState({})
  const [finished, setFinished] = useState(false)
  const [unansweredQuestionIds, setUnansweredQuestionIds] = useState(Array(quizQuestions.length).fill(null).map((u, i) => i ))
  ...
  return (
    <div style={{margin:"1em"}} className="content">
      <div className="intro-y box mt-5">
        ...
        <div className="intro-y box mt-5">
          <div className="items-center p-5 border-b border-gray-200">
            <button className="button w-30 shadow-md mr-1 mb-2 bg-theme-1 text-white" onClick={confirmAnswer}>
              Next Question
            </button>
          </div>
        </div>
